...
func init() {
	...
	viper.SetDefault("serve.cors.allow_all_origins", false)
	viper.SetDefault("serve.cors.allow_origins", []string{"http://localhost:8080"})
	viper.SetDefault("serve.cors.allow_credentials", true)
	viper.SetDefault("serve.cookies_secure", false)
	viper.SetDefault("token.issuer", "jwt-auth")
	viper.SetDefault("token.access_token.expires", "5m")
	viper.SetDefault("token.access_token.transport.cookies", false)
	viper.SetDefault("token.access_token.transport.json_response", true)
	viper.SetDefault("token.refresh_token.expires", "8h00m")
	viper.SetDefault("token.refresh_token.expires_extended", "8760h00m")
	viper.SetDefault("token.refresh_token.transport.cookies", true)
	viper.SetDefault("token.refresh_token.transport.json_response", false)
